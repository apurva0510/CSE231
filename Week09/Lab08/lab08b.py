        ...
        if country and city:
            ...
            for continent in data_map:
                if country in data_map[continent]:
                    # if city is not in the set of cities for the country, add it to the set
...
def display_map(data_map):
    ...
    for continent in continents_list:
        print(f"{continent}:")
        countries_list = sorted(data_map[continent]) #create a sorted list of the countries keys in the continents
        for country in countries_list:
            print(f"{country:>10s} --> ",end = '')
            cities = sorted(list(data_map[continent][country])) #create a sorted list of the cities
            cities_str = ", ".join(cities) # join all cities in the list into a string separated by commas and spaces
            print(cities_str) # print the created string of cities
